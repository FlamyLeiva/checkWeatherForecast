...
```{r}
predicciones_SantaClementina<- predicciones %>%
  filter(orchard== "Santa_Clementina")
clima_SantaClementina <-clima_horario %>%
  filter(orchard== "Santa_Clementina")
```

```{r}
predicciones_SantaClementina%>%
  mutate(datetimePredict= as.POSIXct(datetimePredict, tz = "America/Santiago"))->test
```
...
```{r}
df_comparacion <- predicciones_SantaClementina %>%
  inner_join(clima_SantaClementina, by = c("datetimePredict" = "datetime"))
```
...
Temperauea : En promedio difieren en 1.4 unidades
```{r}
...
Da más peso a los errores más grandes debido al cuadrado de los errores individuales. Es más sensible a valores atípicos.
```{r}
...
Baja correlación con la diferencia horaria. 
...
```
Muy malas correlaciones entre la diferencia horaria y la predicción

Estadisticas generales sin considerar la diferencia horaria:

1. MAPE error porcentual absoluto medio
Va del 0 al infinito. mape_temp buena, mape precip mala.
En promedio varian en un 15% de los valores. --> 

** podría aislarse para cierta diferencia horaria por ejemplo por bloques de 10 horas volver a sacar estadisticas. 

```{r}
mape_temp <- mean(abs((df_comparacion$Predict_tempMean - df_comparacion$tempMean) / df_comparacion$tempMean) * 100, na.rm = TRUE)
mape_precip <- mean(abs((df_comparacion$Predict_precipitation - df_comparacion$precipitation) / df_comparacion$precipitation) * 100, na.rm = TRUE)
...
Muy buea correlación de temperatura. Buen predictor. (Va de -1 a 1 )
```{r}
